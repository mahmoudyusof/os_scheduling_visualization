...
def execute (to_draw_list,ind_list):
    ...
    for i in range (len(to_draw_list)):
        ...
        gnt.annotate('P'+str(ind_list[ i ]) , ( start + to_draw_list[ i ][ 1 ] / 2 , height/2 ),horizontalalignment='center', verticalalignment='center')
        total_waiting_time += (start - to_draw_list[i][0])
    
    gnt.set_xlim(0, total_time_so_far)
    gnt.set_ylim(0, height)
    plt.title("Average waiting time = " + str(total_waiting_time/len(to_draw_list) ) )
